/**
 * Relationship JSON Generator
 *
 * This JavaScript program reads relationship data from a JSON input file,
 * processes the data to classify entities as real or junction tables,
 * and generates a structured JSON file summarizing these entities and relationships.
 *
 * Usage:
 *   node createRelationshipJSON.js <inputFile> <outputFile>
 *
 * Parameters:
 *   inputFile  - Path to the JSON file containing relationship data.
 *   outputFile - Path where the generated JSON file will be saved.
 *
 * Example:
 *   node createRelationshipJSON.js ./input.json ./output/relationshipDatabase.json
 *
 * Dependencies:
 *   - Node.js
 ...
 */
function addRelationship(map, entity1, relationship, entity2, details) {
  const entry = {
    entity1,
    relationship,
    entity2,
...
/**
 * Generates a structured JSON content summarizing the entities and their relationships.
 *
 * @param {Object} data - The input data containing relationship information.
 * @returns {Object} A structured JSON object.
 */
function createRelationshipJSON(data) {
  ...
  // Populate entities with their classification
  structuredJSON.entities = Object.keys(data.allTableFieldsSpec).map(
    (entityName) => ({
      name: entityName,
      type: junctionTableNames.has(entityName) ? 'junction' : 'real',
    }),
  );
...
function main() {
  ...
  if (args.length !== 2) {
    ...
    console.error(
      'Usage: node createRelationshipJSON.js <inputFile> <outputFile>',
    );
    process.exit(1);
  }
  ...
  // Generate the Relationship JSON
  const relationshipOutput = createRelationshipJSON(inputData);

  // Write the JSON content to the output file
  try {
    fs.writeFileSync(
      outputFilePath,
      JSON.stringify(relationshipOutput, null, 2),
      'utf-8',
    );
    console.log(`Success: JSON file generated at "${outputFilePath}".`);
  } catch (error) {
